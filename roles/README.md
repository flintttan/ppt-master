...
| **模板设计师**     | [Template_Designer.md](./Template_Designer.md)             | 页面模板生成（可选）           | 设计规范     | 页面模板 SVG       | 所有格式 |
...
> - Image_Generator 是串行环节，仅当图片方式**包含**「AI 生成」时触发，图片归集完成后才进入 Executor 阶段
...
### 1️⃣+ Template_Designer (模板设计师)

**何时使用**: 用户选择「B) 生成新模板」时

**触发条件**: 可选角色，仅当用户需要生成新模板时调用

**核心能力**:

- 根据设计规范生成 4 个核心模板（封面、章节页、内容页、结束页）
- 可选生成目录页模板
- 使用占位符标记可替换内容
- 确保模板遵循设计规范的配色、字体、布局
